...
```
~/.claude/plugins/cache/mj-agentlab-marketplace/{plugin}/{version}/
```
...
```
正常加载（缓存）：
  settings.json (enabledPlugins: "learn-kit@mj-agentlab-marketplace")
    → installed_plugins.json (installPath: cache/.../learn-kit/1.0.0/)
      → 缓存目录（静态拷贝）
        → Claude Code 加载技能 ← 始终是安装时的旧版本

--plugin-dir 覆盖：
  claude --plugin-dir ../../mj-agentlab-marketplace/feature/xxx/plugins/learn-kit
...
两个仓库需在同级目录下，且**都使用 bare repo worktree 模式**：
...
# 示例：
#   ../../mj-agentlab-marketplace/feature/add-skill-invocation-shortcuts/plugins/
#   ../../mj-agentlab-marketplace/main/plugins/
```
...
cd D:/workspace/10-software-project/projects/mj-agentlab-marketplace/feature/add-skill-xxx
...
- 验证多插件协作场景（如 learn-kit 技能链）
- 验证 marketplace.json 中的插件注册是否正确

### 5.2 完整源切换流程

> **重要**：本地 marketplace 和 GitHub marketplace 使用**相同的名称** `mj-agentlab-marketplace`。注册本地源会**覆盖** `known_marketplaces.json` 中的 GitHub 源条目。测试完毕后**必须恢复**。
...
记录当前 `mj-agentlab-marketplace` 的 source 信息：
```json
{
  "mj-agentlab-marketplace": {
    "source": {
...
验证通过后，在 marketplace 仓库中执行发布。以下是基于实际操作总结的完整清单：
...
#    如有插件变更，也 bump 插件：-Scope "learn-kit" -From "1.0.0" -To "1.1.0"
...
git add plugins/learn-kit/.claude-plugin/plugin.json plugins/learn-kit/CHANGELOG.md  # 如有插件变更
...
`known_marketplaces.json` 中 `mj-agentlab-marketplace` 的 `installLocation` 会指向本地路径。后续 `/plugin update` 会尝试从本地路径拉取更新，而不是 GitHub：
...
| 阶段 | 命令 | 说明 |
|------|------|------|
| **阶段 1** | `claude --plugin-dir ../../mj-agentlab-marketplace/<worktree>/plugins/{plugin}` | 启动时指定本地插件（Bash） |
| | PowerShell: `claude --plugin-dir "D:\...\mj-agentlab-marketplace\<worktree>\plugins\{plugin}"` | PowerShell 需绝对路径 |
| | `/reload-plugins` | 修改后热重载 |
| **阶段 2** | `/plugin uninstall {plugin}@mj-agentlab-marketplace` | 卸载缓存版本 |
| | `/plugin marketplace remove mj-agentlab-marketplace` | 移除 GitHub 源 |
| | `/plugin marketplace add ../../mj-agentlab-marketplace/<worktree>` | 注册本地源（指向 worktree） |
| | `/plugin install {plugin}@mj-agentlab-marketplace` | 从本地源安装 |
| **恢复** | `/plugin marketplace remove mj-agentlab-marketplace` | 移除本地源 |
| | `/plugin marketplace add MJ-AgentLab/mj-agentlab-marketplace` | 恢复 GitHub 源 |
| | `/plugin install {plugin}@mj-agentlab-marketplace` | 重新安装 |
| **阶段 3** | `/plugin marketplace update mj-agentlab-marketplace` | 从 GitHub 更新 marketplace |

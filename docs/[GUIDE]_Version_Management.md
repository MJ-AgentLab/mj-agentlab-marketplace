...
│
├── mj-doc v1.0.0                ← plugins/mj-doc/.claude-plugin/plugin.json（权威源）
│   同步 → marketplace.json plugins[name=mj-doc].version
│
├── mj-git v1.0.0                ← plugins/mj-git/.claude-plugin/plugin.json
├── mj-n8n v1.0.0                ← plugins/mj-n8n/.claude-plugin/plugin.json
└── mj-ops v1.0.0                ← plugins/mj-ops/.claude-plugin/plugin.json
```
...
| mj-doc | `plugins/mj-doc/.claude-plugin/plugin.json` → `version` | `.claude-plugin/marketplace.json` → `plugins[name=mj-doc].version` |
| mj-git | `plugins/mj-git/.claude-plugin/plugin.json` → `version` | 同上模式 |
| mj-n8n | `plugins/mj-n8n/.claude-plugin/plugin.json` → `version` | 同上模式 |
| mj-ops | `plugins/mj-ops/.claude-plugin/plugin.json` → `version` | 同上模式 |
...
| # | 检查项 | 失败示例 |
|---|--------|----------|
| 1 | plugin.json 字段完整 | `ERROR: plugins/mj-git/.claude-plugin/plugin.json missing field: license` |
...
| 4 | 目录结构 | `ERROR: plugins/mj-git missing required file: CLAUDE.md` |
| 5 | 版本一致性 | `ERROR: VERSION (1.1.0) != marketplace.json (1.0.0)` |
| 6 | CHANGELOG 存在性 | `ERROR: plugins/mj-git/CHANGELOG.md not found` |

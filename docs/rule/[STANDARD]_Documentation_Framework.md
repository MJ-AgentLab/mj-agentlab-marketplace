...
|---------|------|---------|
| v1.8 | 2026-07-23 | **§2.7 Category 4 A6 trigger set completion**. v1.7 的 Category 4 只把 `scripts/{generate-nlm-contract,probe-learn-kit-nlm-bridge,resolve-release-state}.mjs` 三个 release/contract 脚本列为 A6 trigger，但同批（#169 release-hardening）落地的 `run-release.mjs`（draft-first release 编排器）与 `release-verify-install.mjs`（pre-publish injected-core install 校验器）**同样 release-critical 却被漏列**——改这两个脚本的 PR 此前不要求同步根 CLAUDE.md。本次把二者补入 §2.7 line 349 枚举 + `scripts/check-a6.mjs` `TRIGGERS` regex（`tests/check-a6.test.mjs` 单测钉死新增两条）。触发面从 3 → 5 个 release 脚本；category 结构（4 类）不变。发现于 v7.0.0 release 收尾：`run-release.mjs` 的 post-createDraft poll 修复（PR #173）本应触发 A6 却未触发，暴露该 gap。**Non-trigger 自身归档**：仅改 §2.7 line 349 枚举 + §5；章节结构不动；v1.7 文件留原路径推进 v1.8。marketplace VERSION 不动（develop 7.0.1 pre-bumped；本次纯 governance/doc PR，非 release）。Adopted in PR (A6-trigger-gap governance). |

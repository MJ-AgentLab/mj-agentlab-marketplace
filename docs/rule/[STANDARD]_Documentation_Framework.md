...
> [!IMPORTANT]
> A6 workflow 的 check context 为 `A6 CLAUDE.md sync`。它要真正阻断 merge，必须被列入对应 branch ruleset 的 required status checks；否则本 gate 只是红叉提示，不构成 blocker。

# notebooklm-kit — NotebookLM Learning-Loop Plugin for Claude Code

> Renamed from `mj-nlm` (formerly hosted on `ranzuozhou/my-marketplace`). v2.4.1 functionality preserved 1:1; plugin identity and namespace updated.
...
>
> **v2.3 — Deprecation removal + 启动规范**：移除 v2.2 forward-announce 的 `/notebooklm-kit:learn` skill；新增 2 份共享规范 `preflight-checklist.md`（启动冒烟三级 L1 Auth / L2 MCP health / L3 Notebook scope）+ `quota-estimation.md`（单调用基线 + 双 wrapper 配额预告）。skill 数 8 → 7；shared 文档 8 → 10。**轻量 BREAKING（v2.2 已公告期 ~2 周）**。
>
> **v2.2 — 高层入口整合**：新增 `/notebooklm-kit:learn-make`（生成学习资料）+ `/notebooklm-kit:learn-test`（生成考察资料）两个 high-level wrapper。用户对外只需记 2 个命令，底层 skill 仍可独立调用。
>
...
| 命令 | 说明 |
|---|---|
| `/notebooklm-kit:learn-make` | **生成学习资料 wrapper**：编排 `build` + `studio` 上游 7 类制品（mind_map / video / slide / audio / report / infographic / data_table）；默认 record mode；支持 `--triple-view` / `--with-download` / `--download-only` / `--resume` |
| `/notebooklm-kit:learn-test` | **生成考察资料 wrapper**：编排 `studio` quiz/flashcards + 可选 `query` Mode D（错题归因）/ F（自检）/ E（来源核查）；默认锁定 quiz + flashcards，其他三项需勾选；支持 `--full` / `--rootcause` / `--selfcheck` / `--sourcecheck` |

### 底层 skill（独立可调用，wrapper 内部调度）

| 命令 | 说明 |
|---|---|
| `/notebooklm-kit:auth` | NLM 认证（登录、刷新、切换账号、故障排查） |
| `/notebooklm-kit:build` | 创建知识库（扫描 → 导入 → 打标签 → 来源充足性预检 → 领域定向报告） |
| `/notebooklm-kit:manage` | 管理知识库（Notebook / Source / Tag CRUD + 分享） |
| `/notebooklm-kit:query` | 知识问答（6 种 Mode：Single / Cross / Deep Research / Quiz Root Cause / Source Check / Self-Check） |
| `/notebooklm-kit:studio` | Studio 制品生成（9 种类型 × 4 view 视角 + 默认幻觉防护 + v2.1 record/download/both 三输出模式，默认 record） |

## v2.2 核心新概念

### 高层入口（learn-make / learn-test）

v2.2 把 v2.0 引入的单一编排器 `/notebooklm-kit:learn` 拆为两个语义对偶的 wrapper：

- **学习侧**：`/notebooklm-kit:learn-make <topic>` —— 从 0 到 1 创建 notebook + 一键生成学习制品组合
- **考察侧**：`/notebooklm-kit:learn-test <notebook_id>` —— 在已有 notebook 上生成考察资料并按需触发归因 / 自检 / 核查
...
```
# Step 1: 生成学习资料
/notebooklm-kit:learn-make DQV
→ build → mind_map / video / slide / audio 4 份 record markdown 进 git

# Step 2: 生成考察资料
/notebooklm-kit:learn-test MJ-system-mod-DQV-20260508
...
`/notebooklm-kit:learn`（v2.0 引入 / v2.2 deprecated）已在 v2.3 移除。所有调用应改用 `/notebooklm-kit:learn-make` + `/notebooklm-kit:learn-test` 串联。Migration 见 `CHANGELOG.md#[2.3.0]`。
...
```bash
claude --plugin-dir "<path-to-mj-agentlab-marketplace>/plugins/notebooklm-kit"
```

## 验证安装

```
/plugin                    # 确认 notebooklm-kit 出现在列表
/notebooklm-kit:learn-make        # 测试 v2.2 新 wrapper 加载
/notebooklm-kit:learn-test        # 测试 v2.2 新 wrapper 加载
...
```
/notebooklm-kit:build              # 仅构建知识库（含 v2 P6/P7）
/notebooklm-kit:studio             # 仅生成单个制品（含 v2 view + guardrails + v2.1 三模式）
/notebooklm-kit:query              # 仅问答（6 种 Mode）
/notebooklm-kit:manage             # 生命周期管理
/notebooklm-kit:auth               # 认证维护
```
...
- "生成学习资料 / 一键学习材料 / 学习编排" → `/notebooklm-kit:learn-make`（v2.2 推荐）
- "生成考察资料 / 出题 / 出 quiz / 学习效果检验" → `/notebooklm-kit:learn-test`（v2.2 推荐）
- "建知识库 / 生成领域定向报告" → `/notebooklm-kit:build`（advanced 单步）
- "生成三版音频 / 零基础版+结构版+挑战版" → `/notebooklm-kit:studio` 或 `/notebooklm-kit:learn-make --triple-view`
- "只要 metadata / 不下载 / 在线引用 / online reference" → `/notebooklm-kit:studio --mode record`（v2.1 默认）
- "metadata + 本地副本 / 学习+归档" → `/notebooklm-kit:studio --mode both` 或 `/notebooklm-kit:learn-make --with-download`
- "我做完 quiz 错了 5 题，分析下" → `/notebooklm-kit:learn-test --rootcause`（v2.2 推荐）或 `/notebooklm-kit:query` Mode D
- "刚才的回答靠谱吗，能找到来源吗" → `/notebooklm-kit:query` Mode E
- "做一次理解度自检" → `/notebooklm-kit:learn-test --selfcheck`（v2.2 推荐）或 `/notebooklm-kit:query` Mode F
...
- ✅ v2.2：高层入口整合（`/notebooklm-kit:learn-make` + `/notebooklm-kit:learn-test`），旧 `/notebooklm-kit:learn` 标 deprecated
- ✅ v2.3：移除 v2.2 deprecated 的 `/notebooklm-kit:learn` skill；新增 preflight-checklist + quota-estimation 两份共享规范

# CLAUDE.md — notebooklm-kit Plugin

## Plugin 概述

notebooklm-kit 是一个 NotebookLM 学习闭环技能家族 Plugin（2026-05 从 mj-nlm 重命名而来；功能 1:1 保留）。由 MJ-AgentLab 组织维护，对外通用，已在 mj-system 与 mj-agent 两个项目实际使用。
...
- v2.3 的 `nlm-shared/preflight-checklist.md` 由文档级模板正式编入 6 个 skill 的 Phase 0 实施段：
...
- 删除 `/notebooklm-kit:learn` skill（v2.0 引入，v2.2 deprecated）；改用 `/notebooklm-kit:learn-make` + `/notebooklm-kit:learn-test` 串联
- 新增 2 份共享规范：
  - `nlm-shared/preflight-checklist.md` — 三级启动冒烟（L1 Auth / L2 MCP health / L3 Notebook scope）
  - `nlm-shared/quota-estimation.md` — 单调用基线 + 双 wrapper 配额预告（v2.0/v2.1 卡 Phase 7 的根因前移到 Phase 0）
- 共享文档总数 8 → 10；skill 总数 8 → 7

**v2.2 升级要点**（高层入口整合，**非破坏性**）：
- 新增 2 个 high-level wrapper skill：
  - `/notebooklm-kit:learn-make` — 生成学习资料（编排 build + studio 上游 7 类制品）
  - `/notebooklm-kit:learn-test` — 生成考察资料（编排 studio quiz/flashcards + 可选 query Mode D/E/F）
- 用户对外只需记 2 个 high-level 命令，底层 skill 仍可独立调用
- 命令形态：`/notebooklm-kit:learn-make <topic>` → `/notebooklm-kit:learn-test <notebook_id>`

**v2.1 升级要点**（默认行为变更，**非破坏性**）：
- studio Phase 4 默认输出**元信息 markdown**（record mode），而非 download 二进制；download 降级为 `--mode download` 显式 opt-in
- 新增共享模板 `nlm-shared/artifact-metadata-template.md`（≤ 50 行 record 范式）
...
| Skill | 命令 | 职责 | v 版本 |
|---|---|---|---|
| **learn-make** | `/notebooklm-kit:learn-make` | **生成学习资料**：编排 build + studio 上游 7 类制品（mind_map/video/slide/audio/report/infographic/data_table） | v2.2 |
| **learn-test** | `/notebooklm-kit:learn-test` | **生成考察资料**：编排 studio quiz/flashcards + 可选 query Mode D（错题归因）/F（自检）/E（来源核查） | v2.2 |

### 底层 skill（独立可调用，wrapper 内部调度）

| Skill | 命令 | 职责 | v 版本 |
|---|---|---|---|
| **auth** | `/notebooklm-kit:auth` | NLM 认证生命周期（登录、刷新、切换、排障） | v1（不动） |
| **build** | `/notebooklm-kit:build` | 知识库创建（扫描 → 导入 → 打标签 → 来源充足性预检 → 领域定向报告） | v2 升级（新增 P6/P7、双项目支持） |
| **manage** | `/notebooklm-kit:manage` | 知识库 CRUD + 分享 | v1（不动） |
| **query** | `/notebooklm-kit:query` | 知识问答（Single / Cross / Deep Research / Quiz Root Cause / Source Check / Self-Check） | v2 升级（新增 Mode D/E/F） |
| **studio** | `/notebooklm-kit:studio` | Studio 制品生成（9 种类型 × 4 view 视角 + 默认幻觉防护 + v2.1 record/download/both 三模式） | v2.1（默认 record mode） |
...
scope 按 project 区分（详见 `skills/nlm-shared/naming-reference.md`）。
...
- **v2.3 起 Phase 0 标准化为三级 preflight**（详见 `nlm-shared/preflight-checklist.md`）：L1 Auth Token / L2 NLM Service Health / L3 Notebook scope（条件触发）
- 认证失败统一引导到 `/notebooklm-kit:auth`
...
- **v2.2 起对外推荐入口为 `/notebooklm-kit:learn-make` + `/notebooklm-kit:learn-test` 两个 wrapper**；底层 5 skill 仍可独立调用（advanced 路径）
- **v2.3 起 preflight 通过后必须按 `nlm-shared/quota-estimation.md` 模板告知用户预计耗时与配额消耗**
- 共享参考资源位于 `skills/nlm-shared/` 目录

## 文件结构

```
skills/
├── auth/        # 认证技能 + troubleshooting 手册（v1）
├── build/       # 知识库创建（v2，含 P6/P7）
├── manage/      # 生命周期管理（v1）
├── query/       # 知识问答（v2，6 种 Mode）
├── studio/      # Studio 制品（v2，含 view + guardrails；v2.1 三输出模式）
├── learn-make/  # 学习侧编排器 wrapper 1（v2.2 新增）
├── learn-test/  # 考察侧编排器 wrapper 2（v2.2 新增）
└── nlm-shared/      # 共享参考资源（10 份）

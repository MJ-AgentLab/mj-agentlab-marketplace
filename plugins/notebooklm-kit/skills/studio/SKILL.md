...
description: >
  ...
  Do not use for: 创建/重建 notebook 与导入 source (use notebooklm-kit:build),
  完整学习闭环（含 build + 多种制品 + quiz + 自检） (use notebooklm-kit:learn),
  知识问答/错题归因/来源核查/理解度自检 (use notebooklm-kit:query),
  notebook 管理或分享 (use notebooklm-kit:manage).
---

# notebooklm-kit:studio
...
**前置 skill**：知识库构建使用 `/notebooklm-kit:build`（v2 起包含 Phase 6 预检 + Phase 7 领域定向报告）。

## Prerequisites

- 已有 notebook（推荐通过 v2 `/notebooklm-kit:build` 创建，包含 `00c-定向-` 报告）
...
4. 无可用 notebook → **H1**（引导先执行 `/notebooklm-kit:build`）
...
2. **检查并读取 `00c-定向-{topic}领域定向报告` source**：
   - 存在：读取"核心概念地图"段
   - 不存在（旧 notebook 或 build 跳过 P7）：**降级**为仅用 notebook_describe 摘要 + AskUserQuestion 让用户手动输入 5-8 个关键概念词；同时提示「建议先跑 `/notebooklm-kit:build --resume <nbid>` 仅触发 P7 补齐 00c」
3. 提取 5-8 个关键概念词
...
| **H4** | Hard Block（**v2 新增**） | 来源充足性预检 BLOCK（如 1 页 PDF 请求 audio_long） | 强制降级 artifact_type 或提示先 `/notebooklm-kit:build` 补来源 |
...
下一步:
  - 生成学习资料完整编排 → /notebooklm-kit:learn-make
  - 生成考察资料完整编排 → /notebooklm-kit:learn-test
  - 继续生成其他制品 → /notebooklm-kit:studio
  - Quiz 错题 root cause / 来源核查 / 理解度自检 → /notebooklm-kit:query

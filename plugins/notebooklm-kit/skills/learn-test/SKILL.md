...
description: >
  ...
  Do not use for: 单步制品生成 (use notebooklm-kit:studio), 生成学习资料制品 (use notebooklm-kit:learn-make),
  单步知识问答 (use notebooklm-kit:query), notebook 创建或导入 source (use notebooklm-kit:build),
  notebook 管理或分享 (use notebooklm-kit:manage), 仅认证修复 (use notebooklm-kit:auth).
...
**v2.2 升级要点**：
- 与 wrapper 1 `/notebooklm-kit:learn-make`（生成学习资料）对偶
- 替代 v2.0/v2.1 `/notebooklm-kit:learn` 的下游考察编排（v2.3 已删除）
- quiz / flashcards 走 studio record mode，进 git；用户在 NotebookLM 在线答题
- 内部 100% 复用 studio / query 现有 Phase，不重写底层逻辑

**前置 skill**：`/notebooklm-kit:learn-make`（先有 notebook + 学习资料，再做考察）；`/notebooklm-kit:auth`（认证）；`/notebooklm-kit:studio` / `/notebooklm-kit:query`（被本 skill 调度，无需用户手调）。
**互补 skill**：`/notebooklm-kit:manage`（生命周期管理）。
...
- 已有 notebook（`/notebooklm-kit:learn-make` 生成的，或单独 `/notebooklm-kit:build` 创建的）
...
| 命令 | 说明 |
|---|---|
| `/notebooklm-kit:learn-test <notebook_id>` | 默认走 Phase 2a (quiz + flashcards 生成)，Phase 1 多选默认锁定 |
| `/notebooklm-kit:learn-test --full <notebook_id>` | Phase 1 多选**默认勾全 5 类**，自动连续走完 quiz → 答题等待 → D → F → E |
| `/notebooklm-kit:learn-test --rootcause <notebook_id>` | 仅走 Phase 2b（错题归因），跳过出题；需用户先准备错题列表 |
| `/notebooklm-kit:learn-test --selfcheck <notebook_id>` | 仅走 Phase 2c（7 项理解度自检 → 99- 仪表盘 Note） |
| `/notebooklm-kit:learn-test --sourcecheck <notebook_id>` | 仅走 Phase 2d（DIRECT/INFERRED/UNCERTAIN/CONTRADICTED 5 级核查） |
...
digraph nlm_learn_test {
    ...
    start -> P0;
    P0 -> H0 [label="无"];
    H0 -> start [label="先 /notebooklm-kit:learn-make"];
    P0 -> P1 [label="有"];
...
2. 匹配结果：
   - **0 个匹配** → **H0**（引导先 `/notebooklm-kit:learn-make <topic>` 创建 notebook 与学习资料）
   - **1 个匹配** → 直接进入 Phase 1
...
| # | 项目 | 调用底层 | 默认勾选 | 说明 |
|---|---|---|---|---|
| 1 | quiz 生成 | `/notebooklm-kit:studio artifact_type=quiz` | ✅ **锁定** | 生成 record markdown，含 NotebookLM URL（用户在 NLM 在线答题）|
| 2 | flashcards 生成 | `/notebooklm-kit:studio artifact_type=flashcards` | ✅ **锁定** | 同上 |
| 3 | 错题归因（Mode D） | `/notebooklm-kit:query` Mode D | ⬜ | 用户在 NLM 答完 quiz 后，提供错题列表 → 归因 + 重学建议 |
| 4 | 理解度自检（Mode F） | `/notebooklm-kit:query` Mode F | ⬜ | 7 项指标对话式自检 → `99-自检-理解度仪表盘-{YYYYMMDD}-{HHMM}` Note |
| 5 | 来源核查（Mode E） | `/notebooklm-kit:query` Mode E | ⬜ | DIRECT/INFERRED/BACKGROUND/UNCERTAIN/CONTRADICTED 5 级标注 |
...
- 如用户需要修改子参数（如题数、难度），引导其单独调用 `/notebooklm-kit:studio`
...
**目标**：调度 `/notebooklm-kit:studio` 生成 quiz + flashcards 两类制品。

```
/notebooklm-kit:studio --mode record artifact_type=quiz on <notebook_id>
/notebooklm-kit:studio --mode record artifact_type=flashcards on <notebook_id>
```
...
**目标**：调度 `/notebooklm-kit:query` Mode D，对用户提供的错题列表做归因。
...
**触发**：
```
/notebooklm-kit:query --mode=D notebook_id=<id> errors=<错题列表>
```
...
**目标**：调度 `/notebooklm-kit:query` Mode F，做 7 项理解度对话式自检。

**触发**：
```
/notebooklm-kit:query --mode=F notebook_id=<id>
```
...
- 三级评级：入门 / 熟悉 / 掌握
- 24h 回访预约（手动触发 `/notebooklm-kit:query --mode=recall`）
...
**目标**：调度 `/notebooklm-kit:query` Mode E，对 Phase 2c 仪表盘中"指标 1-4"用户口述部分（或用户独立指定结论）做来源核查。

**触发**：
```
/notebooklm-kit:query --mode=E notebook_id=<id> claims=<结论列表 或 自动从 Phase 2c 抽取>
```
...
| ID | 类型 | 触发 | 行为 |
|---|---|---|---|
| **H0** | Hard Block | Phase 0 无可用 notebook | 引导先 `/notebooklm-kit:learn-make <topic>`；不允许 wrapper 内部触发 build（避免 wrapper 1/2 职责混淆） |
...
来源核查通过率: {%, N/A 未核查}
24h 回访预约: {YYYY-MM-DD HH:MM, N/A 未自检}（手动触发 /notebooklm-kit:query --mode=recall）

Tag 自动添加: `learn-test-loop`

下一步:
  - 错题指向"边界不清" → 重生 audio(structural) + 二轮 quiz → /notebooklm-kit:learn-make --resume {notebook_id} 选 audio + structural view
  - 24h 后复述 → /notebooklm-kit:query --mode=recall {notebook_id}
  - 进入下一主题 → /notebooklm-kit:learn-make <new_topic>
  - 仪表盘"入门"评级建议重做学习闭环 → /notebooklm-kit:learn-make --resume {notebook_id} + /notebooklm-kit:learn-test --full {notebook_id}
```
...
```
用户：/notebooklm-kit:learn-test MJ-system-mod-DQV-20260508
→ Phase 0: 定位 notebook
...
```
用户：/notebooklm-kit:learn-test --full MJ-system-mod-DQV-20260508
→ Phase 0-1: 默认勾全 5 项
...
→ /notebooklm-kit:learn-test --rootcause MJ-system-mod-DQV-20260506
...
```
用户：DQV 我学完了，做个自检
→ /notebooklm-kit:learn-test --selfcheck MJ-system-mod-DQV-20260506
...
```
用户：/notebooklm-kit:learn-test 医疗合规-2026

...
description: >
  ...
  Do not use for: 仅生成制品 (use notebooklm-kit:studio), 知识问答/错题分析/来源核查 (use notebooklm-kit:query),
  生成学习资料的完整编排（build + studio 上游 7 类制品） (use notebooklm-kit:learn-make),
  生成考察资料的完整编排（quiz/flashcards + 错题/自检/核查） (use notebooklm-kit:learn-test),
  notebook 删除/重命名/分享 (use notebooklm-kit:manage), 仅认证修复 (use notebooklm-kit:auth).
---

# notebooklm-kit:build
...
**互补 skill**：制品生成使用 `/notebooklm-kit:studio`；知识问答 / 错题反馈 / 来源核查 / 理解度自检使用 `/notebooklm-kit:query`；生成学习资料完整编排使用 `/notebooklm-kit:learn-make`；生成考察资料完整编排使用 `/notebooklm-kit:learn-test`；生命周期管理使用 `/notebooklm-kit:manage`。
...
digraph nlm_build {
    ...
    DONE [label="Handoff\n→ /notebooklm-kit:learn-make / studio / query", shape=doublecircle];
...
- 通过 → 进 L2
- **H0a**（Hard Block）：失败 → 引导 `/notebooklm-kit:auth`（按 auth troubleshooting 修复，不自动尝试 refresh，避免吞掉用户应该看到的认证错误）

#### L2: NLM Service Health

调用 `notebook_list()` 检查列权限。
- 通过（200 + array，即使空数组）→ 进 Phase 1
- **H0b**（Hard Block）：`PERMISSION_DENIED` → `/notebooklm-kit:auth`（典型 token scope 问题）
...
| ID | 类型 | 触发条件 | 行为 |
|---|---|---|---|
| **H1** | Hard Block | `server_info()` + `refresh_auth()` 均失败 | 阻断。指引 `nlm login` 或 `save_auth_tokens()`。详见 `/notebooklm-kit:auth` |
...
下一步:
  - 生成学习资料完整编排 → /notebooklm-kit:learn-make （v2.3 推荐入口）
  - 生成考察资料完整编排 → /notebooklm-kit:learn-test
  - 单步生成制品 → /notebooklm-kit:studio
  - 知识问答 / 错题反馈 / 来源核查 → /notebooklm-kit:query
  - 管理维护 → /notebooklm-kit:manage
```
...
→ Handoff: 推荐进入 /notebooklm-kit:learn-make
```

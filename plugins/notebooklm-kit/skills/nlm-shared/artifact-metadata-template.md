...
|---|---|---|---|
| 新生成 artifact（默认 record mode） | `/notebooklm-kit:studio --mode record`（或默认） | 用户提供路径，建议 `learning/<topic>/_nlm/` | studio Phase 4 record 分支自动渲染 |
| 生成学习资料（默认） | `/notebooklm-kit:learn-make`（Phase 3 循环默认走 record） | 同上 | wrapper 编排 studio --mode record |
| 生成考察资料（默认） | `/notebooklm-kit:learn-test`（Phase 2a quiz/flashcards 默认走 record） | 同上 | wrapper 编排 studio --mode record |
| 既要 metadata 又要 download | `/notebooklm-kit:studio --mode both` 或 wrapper `--with-download` | record 写 `_nlm/`，binary 用户自存（不入 git） | 学习+归档场景 |
...
- **`→ ../studio/SKILL.md`** — Phase 4 record / download / both 三分支

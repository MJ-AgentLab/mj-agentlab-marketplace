...
> notebooklm-kit 任何一个 skill 在 Phase 0 进入业务流程前都应跑一次轻量 preflight，避免用户走完一半才命中认证 / 服务问题。
>
> v2.3 把这套 checklist 显式化，统一规范"什么算可以推进 Phase 1"。

## 目的

notebooklm-kit 的子 skill 大多耗时长（build 含 source 导入；studio 制品 30s-3min；wrapper 一次跑 7+ 调用）。**任何在 Phase 0 没发现的故障，都会在用户已等了 1-10 分钟后才暴露**——这是 v2.0/v2.1 学习闭环卡 Phase 7 的根因。
...
| 检查 | 命令 | 通过条件 |
|---|---|---|
| Token 状态 | `notebooklm-mcp__refresh_auth()` 等价的 status 调用 | 返回 OK；token 未过期 |

失败处理：
- 立即退出，引导 `/notebooklm-kit:auth`（按 auth skill troubleshooting 修复）
...
| 检查 | 命令 | 通过条件 |
|---|---|---|
| MCP 进程在跑 | `notebooklm-mcp__server_info()` | 返回 server metadata |
| 列 notebook 权限可用 | `notebooklm-mcp__notebook_list()` | 返回 200 + notebook 数组（即使空数组也算通过） |

失败处理：
- `server_info` 失败 → 提示用户重启 MCP（`uv tool reinstall notebooklm-mcp-cli` 或 `nlm login`）
- `notebook_list` PERMISSION_DENIED → 引导 `/notebooklm-kit:auth`
...
| 检查 | 命令 | 通过条件 |
|---|---|---|
| 该 notebook 可读 | `notebooklm-mcp__notebook_describe(notebook_id)` | 返回 notebook metadata |
| query 权限可用（高敏感场景） | `notebooklm-mcp__notebook_query(notebook_id, "ping")` 或类似 noop | 返回 ≥1 字符串响应（不关心内容） |

失败处理：
- notebook_describe NOT_FOUND → 让用户确认 notebook_id；可能是用户记错或 notebook 被删
- notebook_query PERMISSION_DENIED → 这是 v2.0 学习闭环卡 Phase 7 的典型故障；走 `/notebooklm-kit:auth` 重新认证
...
- **跨 turn / 跨 skill 调用**：preflight 重新执行；用户在 Claude Code 一个 session 内多次调用 `/notebooklm-kit:learn-make` / `/notebooklm-kit:build` 等，每次进 Phase 0 都会重跑一次
...
由于不存在技术 cache，无需 `--force-recheck` flag——跨 turn 自动重跑。同 turn 内若用户切了账号（`nlm login switch`），调一次 `/notebooklm-kit:auth` 即可在 Claude conversation 中刷新认知。
...
|---|---|---|---|
| `notebooklm-kit:auth` | 已有完整自查流程，不需 preflight | — | — |
| `notebooklm-kit:build` | Phase 0（替代原 Auth Check） | L1 + L2 | ✅ 显式 Phase 0 段 + 三 H-point (H0a/b/c) |
| `notebooklm-kit:manage` | Phase 0（Workflow 后插入） | L1 + L2 | ✅ 显式 Phase 0 段 |
| `notebooklm-kit:query` | Phase 0；L3 由 Phase 1 首次 query 隐式覆盖 | L1 + L2 + (L3 条件) | ✅ 显式 Phase 0 段 |
| `notebooklm-kit:studio` | Phase 0；L3 由 Phase 1 `notebook_describe` 隐式覆盖 | L1 + L2 + L3 | ✅ 显式 Phase 0 段 |
| `notebooklm-kit:learn-make` | Phase 0 Notebook Locate 前置注解（隐式 preflight） | L1 + L2 + (L3 条件) | ✅ 注解 + 委托 build/studio 子 skill 完整 preflight |
| `notebooklm-kit:learn-test` | Phase 0 Notebook Locate 前置注解（隐式 preflight） | L1 + L2 + L3 | ✅ 注解 + 委托 studio/query 子 skill 完整 preflight |
...
| H | 触发 | 行为 |
|---|---|---|
| **H0a** | L1 fail | Hard block → `/notebooklm-kit:auth` |
| **H0b** | L2a fail | Hard block → 用户重启 MCP |
| **H0c** | L2b PERMISSION_DENIED | Hard block → `/notebooklm-kit:auth` |
| **H0d** | L2b 其他错误 | Soft warn → 报错 + 用户判断是否继续 |
| **H0e** | L3 NOT_FOUND | Hard block → 用户确认 notebook_id |
| **H0f** | L3 PERMISSION_DENIED | Hard block → `/notebooklm-kit:auth`（典型 token scope 问题） |
...
- `→ ../auth/SKILL.md` — Level 1/2 失败时的修复路径

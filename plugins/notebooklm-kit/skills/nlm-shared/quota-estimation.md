...
> notebooklm-kit 大多数操作通过 NotebookLM 的网络/AI 调用完成，单次 wrapper 可能消耗 5-30 分钟。Phase 0 的 preflight 通过后，必须告知用户预计耗时与调用次数，让用户决定是否启动、是否后台等。
>
...
### `/notebooklm-kit:learn-make <topic>`（v2.2 新增 wrapper 1）
...
### `/notebooklm-kit:learn-test <notebook_id>`（v2.2 新增 wrapper 2）
...
### `/notebooklm-kit:studio <notebook_id>`（底层）

单次单制品：见上方 [Per-artifact](#per-artifactstudio_create-完成时长) 表。

### `/notebooklm-kit:query <notebook_id>`（底层）

...
description: >
  ...
  Do not use for: 创建/重建 notebook 或导入 source (use notebooklm-kit:build),
  生成新制品（音频/视频/幻灯片/quiz）(use notebooklm-kit:studio),
  完整学习闭环编排（包含 build + studio + query 编排） (use notebooklm-kit:learn),
  notebook 删除/重命名/分享 (use notebooklm-kit:manage).
---

# notebooklm-kit:query
...
**前置 skill**：知识库构建使用 `/notebooklm-kit:build`；制品生成使用 `/notebooklm-kit:studio`；生成学习资料编排使用 `/notebooklm-kit:learn-make`；生成考察资料编排使用 `/notebooklm-kit:learn-test`。

## Prerequisites

- 已有 notebook（推荐 v2 build 创建，含 `00c-定向-` 报告 + `00d-预检-` 报告）
- NLM MCP 服务已认证（认证问题参考 `/notebooklm-kit:auth`）
- Mode D 需要先有 Quiz 制品（`/notebooklm-kit:studio` artifact_type=quiz）+ 用户答题记录
...
digraph nlm_query {
    ...
    H1 -> P1 [label="先 /notebooklm-kit:build"];
...
skill 在结果末尾建议：
- 重生成针对性制品（call `/notebooklm-kit:studio` 用建议的 prompt）
- 重做 quiz（call `/notebooklm-kit:learn-test --rootcause <nb_id>` 生成新一组 quiz 验证已学）
- 回到 `/notebooklm-kit:learn-make` 重新生成学习资料（如多个错题指向同一概念，需重做 mind_map / video / slide）
...
**写 Note 责任边界**：
- 用户**直接**调用 `/notebooklm-kit:query --mode=self-check` → 由 query 写 Note
- 通过 `/notebooklm-kit:learn-test` wrapper 编排（Phase 2c 自检分支）调用 → query 仅返回结构化结果（dict / JSON），由 learn-test 统一汇总并写 Note（避免双写）
...
```
/notebooklm-kit:query --mode=recall <notebook_id>
```
...
| ID | 类型 | 触发 | 行为 |
|---|---|---|---|
| **H1** | Conditional | `notebook_list()` 为空或无匹配 | 引导先 `/notebooklm-kit:build` |
...
→ 用户接受建议 → 转入 /notebooklm-kit:studio
```
...
```
用户（24h 后）：/notebooklm-kit:query --mode=recall MJ-system-mod-DQV-20260505

...
description: >
  ...
  Do not use for: 单步建库（use notebooklm-kit:build）, 单步制品生成（use notebooklm-kit:studio）,
  知识问答（use notebooklm-kit:query）, 生成考察资料 quiz/flashcards 与错题归因/自检/来源核查（use notebooklm-kit:learn-test）,
...
**v2.2 升级要点**：
- 与 wrapper 2 `/notebooklm-kit:learn-test`（生成考察资料）对偶
- 替代 v2.0/v2.1 的 `/notebooklm-kit:learn` 上游编排（v2.3 已删除）
- 默认 record mode（v2.1 记忆透传），二进制不入 git
- 内部 100% 复用 build / studio 现有 Phase，不重写底层逻辑

**前置 skill**：`/notebooklm-kit:auth`（认证）；`/notebooklm-kit:build`、`/notebooklm-kit:studio`（被本 skill 调度，无需用户手调）。
**互补 skill**：`/notebooklm-kit:learn-test`（生成完学习资料后接续的考察侧入口）；`/notebooklm-kit:manage`（生命周期管理）。
...
- notebooklm-kit 5 个底层 skill 可用（auth / build / manage / studio / query / learn）
...
| 命令 | 说明 |
|---|---|
| `/notebooklm-kit:learn-make <topic>` | 完整流程（无 notebook 时自动 build → 制品多选 → studio 循环） |
| `/notebooklm-kit:learn-make --resume <notebook_id>` | 已有 notebook，跳到 Phase 2 制品多选 |
| `/notebooklm-kit:learn-make --triple-view <topic>` | 选中制品三版连续生成（foundation / structural / challenge） |
| `/notebooklm-kit:learn-make --with-download <topic>` | 默认 record + 同时下载二进制（透传 studio --mode both） |
| `/notebooklm-kit:learn-make --download-only <topic>` | 跳过 record 仅下载（透传 studio --mode download，v2.0 兼容） |
...
digraph nlm_learn_make {
    ...
    DONE [label="Handoff: record markdown 路径列表\n→ 提示下一步 /notebooklm-kit:learn-test", shape=doublecircle];
...
**目标**：当无可用 notebook 时，触发 `/notebooklm-kit:build` 完整流程创建 notebook。
...
**v2.2 注意**：本 wrapper 不复刻 v2.0 learn 的 Gate 1（00c 审定）——审定动作下沉到 build skill 内部 Phase 7 的 H-point；如用户希望强审定，建议直接用 `/notebooklm-kit:build` 单独建库后再 `learn-make --resume <nb_id>`。
...
如用户需要修改子参数，引导其单独调用 `/notebooklm-kit:studio`（advanced 路径）。
...
**目标**：按 Phase 2 选中类型循环触发 `/notebooklm-kit:studio`。
...
```
/notebooklm-kit:studio --mode <mode> --view <view> <artifact_type> on <notebook_id>
```
...
下一步:
  - 生成考察资料（quiz / flashcards / 错题归因 / 自检 / 来源核查）→ /notebooklm-kit:learn-test {notebook_id}
  - 单步追加某类学习制品 → /notebooklm-kit:studio
  - 修订幻灯片（仅 slide_deck） → /notebooklm-kit:studio + studio_revise
  - 补充 source 后重新触发 P6/P7 → /notebooklm-kit:build --resume {notebook_id}
```
...
```
用户：/notebooklm-kit:learn-make DQV
→ Phase 0: notebook_list 无 DQV 匹配 → 进 Phase 1
→ Phase 1: 调 /notebooklm-kit:build → 引导 source 上传 → 命名 MJ-system-mod-DQV-20260508 → 生成 00a/00b/00c/00d
→ Phase 2: AskUserQuestion → 用户选 mind_map + video + slide_deck + audio (4 类)
→ Phase 3: 循环调 /notebooklm-kit:studio (mode=record):
...
→ Handoff: 4 份 record markdown 进 git；提示下一步 /notebooklm-kit:learn-test MJ-system-mod-DQV-20260508
```

### 示例 2：--resume 复用已有 notebook

```
用户：/notebooklm-kit:learn-make --resume MJ-system-mod-DQV-20260506
...
```
用户：/notebooklm-kit:learn-make --triple-view "Rust 所有权"
...
```
用户：/notebooklm-kit:learn-make --with-download DQV
→ Phase 0-2 同示例 1
...
```
用户：/notebooklm-kit:learn-make 1页PDF主题
...
→ 用户选「补来源」→ 引导 /notebooklm-kit:build --resume 重跑 P6/P7 → 通过后重 Phase 3
```
...
- **(historical: ../mj-nlm-learn/SKILL.md)** — v2.2 deprecated 旧编排器，v2.3 已删除（保留历史记录）

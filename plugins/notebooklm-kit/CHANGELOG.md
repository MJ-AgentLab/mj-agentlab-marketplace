...
| 场景 | v2.3 行为 | v2.4 行为 |
|---|---|---|
| 调用 `/notebooklm-kit:build <topic>` | Phase 0 仅 server_info + refresh_auth | Phase 0 升级为 L1+L2，认证或 NLM 健康问题在 Phase 0 阻断而非 Phase 1+ |
| 调用 `/notebooklm-kit:studio <nb_id>` | 无显式 Phase 0；Phase 1 隐式 notebook_list | Phase 0 显式 L1+L2 段；Phase 1 不变 |
| 调用 `/notebooklm-kit:learn-make <topic>` | Phase 0 = Notebook Locate（无 preflight 文字） | Phase 0 头部加 v2.4 隐式 preflight 注解（行为不变） |
...
- **`mj-nlm-learn/` skill**（连同 SKILL.md 整个目录）— v2.0 引入的单一编排器；v2.2 起 deprecated；v2.3 完整移除。所有调用应改用 `/notebooklm-kit:learn-make` + `/notebooklm-kit:learn-test` 串联（v2.2 已上线）。
...
- **README.md** 中「学习闭环编排器 `/notebooklm-kit:learn`（v2.2 deprecated）」整段删除；新增「v2.3 核心新概念」段；"单步使用 advanced" 表删 `/notebooklm-kit:learn` 行
...
| v2.2 状态 | v2.3 行为 |
|---|---|
| 用户调 `/notebooklm-kit:learn` | skill 已不存在 → Claude Code 报 unknown skill；改用 `/notebooklm-kit:learn-make` + `/notebooklm-kit:learn-test` |
| 调 `/notebooklm-kit:learn --resume <nb_id>` | 改用 `/notebooklm-kit:learn-make --resume <nb_id>`（共享 `learn-phase:G{N}-passed` tag） |
| Claude 引擎自动选 `/notebooklm-kit:learn` | 已删除，引擎自动改选 wrapper（learn-make 与 learn-test 的 description 已涵盖原 learn 触发关键词） |
...
把 v2.0 引入的单一编排器 `/notebooklm-kit:learn` 拆为两个语义对偶的 high-level wrapper：`/notebooklm-kit:learn-make`（生成学习资料）+ `/notebooklm-kit:learn-test`（生成考察资料）。用户对外只需记 2 个命令，底层 6 skill 仍可独立调用。**非破坏性 UX 优化**。

### Added

- **新增 skill `learn-make`**（wrapper 1，学习侧编排器）：
...
- **新增 skill `learn-test`**（wrapper 2，考察侧编排器）：
...
- **mj-nlm-learn SKILL.md frontmatter** description 前缀加 `[DEPRECATED v2.2 — use /notebooklm-kit:learn-make + /notebooklm-kit:learn-test 串联替代]`
...
- **`/notebooklm-kit:learn`** — v2.0 引入的单一编排器；v2.2 起标 deprecated，由 `/notebooklm-kit:learn-make` + `/notebooklm-kit:learn-test` 串联替代；保留至 v2.3 删除（约 2-3 周观察期），现有 v2.0/v2.1 用户不受影响

### Migration（v2.1 → v2.2）

| v2.1 `/notebooklm-kit:learn` 阶段 | v2.2 替代 |
|---|---|
| Phase 0-1（来源准备 + build） | `/notebooklm-kit:learn-make <topic>` 内置 Phase 1 (build 条件触发) |
| Gate 1（00c 审定） | 下沉到 build skill Phase 7 H-point；如需强审定可单独 `/notebooklm-kit:build` |
| Phase 2-5（mind_map / video / slide / audio） | `/notebooklm-kit:learn-make` Phase 2 多选 + Phase 3 循环 |
| Phase 6（quiz + flashcards） | `/notebooklm-kit:learn-test <nb_id>` Phase 2a（默认锁定） |
| Phase 7（Mode D 错题归因） | `/notebooklm-kit:learn-test --rootcause <nb_id>` 或 Phase 1 勾选 |
| Phase 8（Mode F 自检） | `/notebooklm-kit:learn-test --selfcheck <nb_id>` 或 Phase 1 勾选 |
| Phase 9（Mode E 来源核查） | `/notebooklm-kit:learn-test --sourcecheck <nb_id>` 或 Phase 1 勾选 |
| `--triple-view` | `/notebooklm-kit:learn-make --triple-view <topic>` |
| `--with-download` / `--download-only` | 同名标志，wrapper 1/2 均支持透传 |
| `--resume <nb_id>` | `/notebooklm-kit:learn-make --resume <nb_id>`（兼容旧 `learn-phase:G{N}-passed` tag） |
| `--lite` / `--quiz-only` | wrapper 1 不选 video / wrapper 2 默认即等价 |

完整 v2.2 替代流程：`/notebooklm-kit:learn-make <topic>` → notebook + 学习资料 → `/notebooklm-kit:learn-test <nb_id>` → 考察资料 + 自检

### Known Issues / Roadmap

- v2.3 计划删除 deprecated 的 `/notebooklm-kit:learn`（约 2-3 周观察期后）
...
- 旧 v2.0/v2.1 `/notebooklm-kit:learn --resume <nb_id>` 流程可由 `/notebooklm-kit:learn-make --resume` 无缝接管（共享 `learn-phase:G{N}-passed` tag）
...
- **保留 v2.0 行为**：单步 `/notebooklm-kit:studio` 调用加 `--mode download`；编排器 `/notebooklm-kit:learn` 加 `--download-only` 或 `--with-download`（推荐后者，留下 record 沉淀）
...
- **新增 skill `mj-nlm-learn`**：学习闭环编排器（10 Phase + 5 Gate），一站式调度 build / studio / query 走完完整学习闭环
  - 命令形态：`/notebooklm-kit:learn <topic>` / `--resume <nbid>` / `--quiz-only <nbid>` / `--triple-view <topic>` / `--lite <topic>`
...
- 现有 v1 notebook 无 `00c` / `00d` 元 source —— 可手动跑 `/notebooklm-kit:build --resume <nbid>` 仅触发 P6/P7 补齐
...
- 24h 复述指标依赖人工触发 `/notebooklm-kit:query --mode=recall`，v2.1 计划用 hooks 自动提醒

<#
.SYNOPSIS
    Decrypt secrets and set OS environment variables for mj-ops plugin

.DESCRIPTION
    Decrypts config/secrets-ops.enc using a team-shared password,
    writes secrets to .env in the plugin root, and sets each variable
    as a User-level OS environment variable.

    The decrypted secrets-ops.conf is automatically deleted after use.
...
.EXAMPLE
    .\scripts\setup-ops-env.ps1
    .\scripts\setup-ops-env.ps1 -Force
...
# Verify we are inside the mj-ops plugin
...
# ── Header ────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "MJ-Ops Plugin — Environment Setup" -ForegroundColor Cyan
Write-Host ("-" * 60)
...
try {
    ...
    # ── Write .env ────────────────────────────────────────────────────
    $envLines = @()
    $envLines += "# MJ-Ops Plugin — auto-generated by setup-ops-env.ps1"

---
name: init
description: Initialize a learning subsystem in this project — scaffolds learning/ folder with 8-stage pedagogical methodology, NLM artifact record template, and INDEX. Use when starting pedagogical documentation for rule-list-based knowledge in a fresh project.
...
│   └── NLM_RECORD_TEMPLATE.md            # from templates/NLM_RECORD_TEMPLATE.md
└── _archive/
    └── .gitkeep                          # empty placeholder
```

Source files live in this skill's `templates/` directory (loaded via `${CLAUDE_PLUGIN_ROOT}/skills/init/templates/`).
...
2. **Study** the worked example at `<plugin-root>/skills/init/references/rfc-2119-keywords-pedagogy.md` — a complete walkthrough applying the 8 stages to RFC 2119 keywords (MUST / SHOULD / MAY etc.).
...
6. **Optional NLM integration**: install the sibling `notebooklm-kit` plugin and run `/notebooklm-kit:learn-make <topic>` to generate audio / video / slide / mind-map artifacts. Record metadata to `learning/<topic-slug>/_nlm/<artifact>.md` using the NLM_RECORD_TEMPLATE.
...
- `templates/NLM_RECORD_TEMPLATE.md` — NLM artifact record schema (paired with notebooklm-kit)
...
- This skill does **not** walk the user through the 8 stages interactively. The methodology is for the user to read and apply manually.
- This skill does **not** invoke any LLM-driven content generation. NLM integration is delegated to the `notebooklm-kit` plugin.

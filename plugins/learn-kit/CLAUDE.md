# CLAUDE.md — learn-kit Plugin (v4.0.0+)
...
> `skills/<name>/SKILL.md` instead. **Dual-host invocation**: Claude Code `/learn-kit:<skill>`,
> Codex `$learn-kit:<skill>` (the bare `$<skill>` never resolves). v4.0.0 is **NLM-only BREAKING**
> — local Markdown / HTML / glossary / concept flows are unchanged.
...
├── .mcp.json                     # notebooklm-mcp server → command learn-kit-nlm-bridge (NLM 部分需要)
├── nlm-bridge/                   # learn-kit-nlm-bridge Python package (契约快照 + runner)
├── scripts/install-nlm-bridge.mjs # 用户手工安装器 (hashed wheel + receipt + two shims)
├── CLAUDE.md                     # 本文件
├── README.md                     # 用户指南
...
└── skills/                       # v3.2.0 起 3 个 skill
    ...
    │   ├── scripts/              # hash-upload-corpus.mjs (staging + hashing helper)
...
`.mcp.json` 注册 `notebooklm-mcp` server；**v4.0.0 起其 `command` 指向本仓的 `learn-kit-nlm-bridge` 桥接器**（server key / wrapper / 空 `args` 不变）。

## 命名约定 · dual-host 全限定调用

本插件 3 个 skill（`three-views` / `glossary` / `concept`）**统一使用全限定形式**：

- **Claude Code**：`/learn-kit:three-views` / `/learn-kit:glossary` / `/learn-kit:concept`
- **Codex**：`$learn-kit:three-views` / `$learn-kit:glossary` / `$learn-kit:concept`（Codex 按 `plugin:skill` 注册，裸 `$three-views` 不解析）

- **历史动机**：marketplace v5.0.0 起本插件 scaffold skill 重命名为 `scaffold-learning`（原 `init` 与宿主 `/init` 撞名）；v6.0.0 起 scaffold 整体退场。全限定约定继承自 v2.0.0 + v3.0.0 ADR 的未来防御思路（避免与未来内置同名冲突）

## 触发 three-views

- Claude Code：`/learn-kit:three-views <topic>`
- Codex：`$learn-kit:three-views <topic>`
...
5. **执行选中项**:
   ...
   - NLM（v4.0.0 Gate A/B）：本地 quota right-sizing → helper `--stage`（冻结上传语料 + `corpus_sha256`）→ `--nlm-preflight`（缺桥接器则撤销 NLM、保本地产物）→ **Gate A**（远端网络同意）→ bounded discovery（`notebook_list` 一次 + 可选 reuse 探测，冻结快照）→ **Gate B**（mutation 同意；逐 mutation 复验 contract + manifest）→ `notebook_create` → `source_add` × `len(nlm_upload_tiers)` → `studio_create` 循环（bounded polling 12×10s；**无 per-call `refresh_auth`**——桥接器管 auth，`AUTH_REQUIRED` 只提示 `nlm login`）→ 终端 URL 表格
...
- **NLM 节奏建议**：默认全勾 (3 + 3 + 3 + 1 = 10 artifact) ≈ 50% 日 quota；Step 4 各 cell 独立可控制粒度；Step 5B Quota gate 的「Reduce subset」/「Pick single tier」再次缩小批量（v4.0.0 前移到 Gate A 之前）；中途 `AUTH_REQUIRED` 只会停下提示你自行 `nlm login`（桥接器不自动登录、不刷 token），长跑前先在终端 `nlm login` 一遍更稳
...
- **(v4.0.0) Re-run guard 只认 corpus hash**：reuse 现有 notebook 仅在其远端语料能与本地 `corpus_sha256` 精确匹配时提供；否则默认新建 timestamped notebook 或 abort（删掉了不安全的 `(topic, len(sources))` 等价 fallback，避免用 3-source notebook 给 1-tier 产物）
- **续跑**：corpus hash 匹配时 reuse，Gate B 只列快照中确实缺失的 mutation（`studio_status` 已生成的 (type, tier) 对自动跳过）
- **mind_map view-agnostic**：dogfood 验证 NLM 媒介对 mind_map 无视 view 差异化指令；单一 shared，可独立要也可与 view-cycled 一起要；与 generated_tiers 数量解耦（1-3 tier 都是 1 个 mind_map）。v4.0.0：mind_map 的 `studio_create` 只传 `source_ids` + `title` + `confirm`，不接受 focus/language
- **infographic 永久丢失提醒**：v6.0.0 删；用户依赖请用 NLM web UI 手动建（per `[ADR]_LearnKit_Consolidation_To_Single_Skill` §3.2）

## NLM 集成 · 工具前缀（v4.0.0）

`.mcp.json` 注册 `notebooklm-mcp` server，但其 `command` 自 v4.0.0 起指向本仓发布的 **`learn-kit-nlm-bridge`**（不再是上游 `notebooklm-mcp`）。桥接器在 initialize/tools/list 阶段只返回 checked-in 契约，首次真实 tool call 才 lazy-start 固定版本 connector 0.8.7。Claude Code 加载后，MCP 工具命名规则为：

```
mcp__plugin_<plugin.json-name>_<.mcp.json-server-key>__<tool>
```

代入 plugin name = `learn-kit`、server key = `notebooklm-mcp`，所以 three-views SKILL.md `allowed-tools` 使用 `mcp__plugin_learn-kit_notebooklm-mcp__*` 前缀。**这些是 Claude Code 的权限前缀**；Codex 不把 `allowed-tools` 当权限边界。

v4.0.0 起 surface 收窄为**恰好 6 个** business tool（桥接器与 `allowed-tools` 双侧一致）：`notebook_list` / `notebook_get` / `notebook_create` / `source_add` / `studio_create` / `studio_status`。外加一条 scoped Bash 权限 `Bash(node "${CLAUDE_SKILL_DIR}/scripts/hash-upload-corpus.mjs" *)`（upload-corpus staging/hash + NLM preflight helper；非通用 Bash）。三个工具**有意排除**：

- **`refresh_auth`** —— 会主动校验 default profile、可能触发 headless auth；桥接器不暴露也不调用（auth 完全由桥接器管理，`AUTH_REQUIRED` 时只提示用户自行 `nlm login`）
- **`server_info`** —— 会产生远端探测
- **`source_delete`** —— 不需要的破坏性能力（re-run guard 默认新建 timestamped notebook 而非删源）

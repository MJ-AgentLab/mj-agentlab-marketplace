<#
.SYNOPSIS
    Decrypt secrets and set OS environment variables for mj-sys-git plugin

.DESCRIPTION
    Decrypts config/secrets-sys-git.enc using a team-shared password,
    writes secrets to .env in the plugin root, and sets each variable
    as a User-level OS environment variable.

    The decrypted secrets-sys-git.conf is automatically deleted after use.
...
.EXAMPLE
    .\scripts\setup-sys-git-env.ps1
    .\scripts\setup-sys-git-env.ps1 -Force
...
# Verify we are inside the mj-sys-git plugin
...
# ── Header ────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "MJ-Sys-Git Plugin — Environment Setup" -ForegroundColor Cyan
Write-Host ("-" * 60)
...
try {
    ...
    # ── Write .env ────────────────────────────────────────────────────
    $envLines = @()
    $envLines += "# MJ-Sys-Git Plugin — auto-generated by setup-sys-git-env.ps1"

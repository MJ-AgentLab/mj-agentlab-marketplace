<#
.SYNOPSIS
    Decrypt secrets and set OS environment variables for mj-sys-ops plugin

.DESCRIPTION
    Decrypts config/secrets-sys-ops.enc using a team-shared password,
    writes secrets to .env in the plugin root, and sets each variable
    as a User-level OS environment variable.

    The decrypted secrets-sys-ops.conf is automatically deleted after use.
...
.EXAMPLE
    .\scripts\setup-sys-ops-env.ps1
    .\scripts\setup-sys-ops-env.ps1 -Force
...
# Verify we are inside the mj-sys-ops plugin
...
# ── Header ────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "MJ-Sys-Ops Plugin — Environment Setup" -ForegroundColor Cyan
Write-Host ("-" * 60)
...
try {
    ...
    # ── Write .env ────────────────────────────────────────────────────
    $envLines = @()
    $envLines += "# MJ-Sys-Ops Plugin — auto-generated by setup-sys-ops-env.ps1"
